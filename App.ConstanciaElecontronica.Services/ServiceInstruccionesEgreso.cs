...
{
    public class ServiceInstruccionesEgreso
	{

		private readonly string _APIconstancia;
		private HttpClient clientAPI;

		public ServiceInstruccionesEgreso(string APIconstancia) {
			_APIconstancia = APIconstancia;
		}


		public async Task<ResponseBase<bool>> EnviarFirma(RequestFirmaInstruEgreso requestFirma, string token)
		{
			...
			try
			{
				...
				using (clientAPI)
				{
					...
					var reponse = await clientAPI.PostAsync($"{_APIconstancia}/api/v1/InstruccionesEgreso", content);
